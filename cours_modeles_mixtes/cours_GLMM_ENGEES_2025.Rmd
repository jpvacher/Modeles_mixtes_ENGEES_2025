---
title: "Modèles linéaires mixtes et modèles linéaires généralisés mixtes"
...
- (Bonus) Générer un plan d'échantillonnage
...
<font size=5>
L'IC est de nouveau un peu étroit. De plus, difficulté d'interprétation car l'intercept correspond à la valeur de la réponse dans le bloc de référence quand le prédicteur est 0.

Regardons comment le jeu de données est construit : 
on a simule des valeurs populationnelles pour mean et alpha, donc il faut que l'inference se situe aussi a l'echelle populationnelle.
</font>
...
- Variables explicatives (effets fixes) = hauteur, surface terrierre, essence dominante
...
Pas de répétitions. Mais les données sont structurées, on peut considérer les massifs comme une strate.

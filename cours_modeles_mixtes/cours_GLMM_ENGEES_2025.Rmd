...
- Quand on envisage une stratégie d'échantillonnage en écologie, le plus souvent on utilise des blocs de mesures (ou strates) : sites de mesures (placette, transect…), mesures répétées (plusieurs années de mesures ou plusieurs mesures par individu), régions naturelles ou biogéographiques.

- Ces structures par blocs nuisent à l’estimation des paramètres de modèles linéaires et doivent donc être prises en compte dans l'analyse.

- De plus, pour diverses raisons inhérentes aux contraintes du travail de terrain, les relevés de terrain sont souvent hétérogènes, ce qui résulte dans des désiquilibres entre blocs. Ceci peut être un obstacle à l’inférence (i.e. certains points captent la variance).
...
- Évaluer si les données que l'on a récoltées sont compatibles avec cette approche : bien réfléchir à la stratégie d'échantillonnage et au protocole de relevés en amont ! Ces deux éléments découlent de la question scientifique posée.
...
- 20 strates (ou blocs, c'est synonyme)
- Une variable réponse, distribution normale de moyenne 5 et sd de 10
...
On écrit un modèle linéaire pour évaluer l'influence de la variable explicative X (Prédicteur) sur la variable Y (Réponse). C'est un modèle “naïf”.
...
Cela semble OK (rédisus répartis sans clusters, sans patron particulier, répartis selon une normale et pas d'effet levier).

On regarde les résidus moyens par strate :
...
<font size=5>
L'IC est de nouveau un peu étroit. De plus, difficulté d'interprétation car l'intercept correspond à la valeur de la réponse dans la strate de référence quand le prédicteur est fixé à 0. On perd une quantité estimée qui nous intéresse : la valeur de la réponse à la valeur moyenne du prédicteur dans l’ensemble de la populations, c'est-à-dire l'ensemble des strates.
...
- Effet aléatoire (toujours qualitatif) :
...
Exemple théorique avec une variable de réponse qui correspond à une mesure de richesse spécifique, donc un dénombrement (pas de valeur négative, que des entiers positifs).
...
- Partie déterministe du modèle :
\[ log(\lambda_i,t) = \alpha + \beta \times Années_i + \varepsilon_{i,j}  \]
- Effet aléatoire :
\[ \varepsilon_{i} \sim N(\mu_i, \sigma_i) \]

\[ \mu_i \sim N(\theta_j, \gamma_j) \]

\[ \theta_j \sim N(0, \upsilon) \]

<font size="4"> i = unité d'échantillonnage ( = strate 1) ;</font>
<font size="4"> j = strate intermédiaire ( = strate 2) ;</font>
...
Note : Les facteurs aléatoires sont toujours des **variables qualitative** (facteurs, ou catégories)
...
Exemple de jeu de données, en colonnes (g) et en lignes (d) (même chose structuré différemment). On choisira la structure en lignes pour ajuster un modèle mixte :
<div class="col2">
...
\[RS \sim Poisson(\lambda_{i})  \]
- Partie déterministe du modèle :
\[ log(\lambda_i) = \alpha + \beta \times Années_i + \varepsilon_{i,j}  \]
\[ \varepsilon_{i} \sim N(\mu_j, \sigma) \]
...
### Bloc “Fixed effects”
Attention à l'interprétation des p-values, dans les modèles complexes, elles n'ont pas nécessairement de valeur. OK dans les modèles simples avec fort degré de liberté.
...
Il s’agit de la variance sur l’échelle du prédicteur linéaire, autrement dit, la variance de la réponse transformée par la fonction de lien dans le cas de GLMM.
...
Ici, les massifs sont considérés comme un effet fixe (variable catégorielle)
...
Ici, les massifs sont considérés comme un terme aléatoire
...
Cela signifie que la richesse moyenne est à 13.37 quand sHdom est à 0, et 15.26 en moyenne quand sHdom passe à 1 dans l'échelle naturelle des valeurs de la variable de réponse car la fonction de lien est pour une distribution de Poisson est log.
...
Dans notre modèle, l’effet aléatoire s’additionne à l’intercept : la structure hiérarchique de l’échantillonnage n’impacte que les différences de richesses inter-massifs.

On a cependant des raisons de penser que les effets fixes sont légèrement différents d’un massif à l’autre, car la composition des peuplements est différente, il peut y avoir des interférence avec des effets paysagers, les espèces d’oiseaux partiellement différentes, …

Dans ce cas, on pourrait opter pour un GLM avec interactions (voir script). Certaines interactions ne sont pas estimables (NA dans le summary du modèle), car il y a un déséquilibre dans le plan d'échantillonnage). On voit qu'il y a des effets par massifs, mais ils ne nous intéressent pas en tant que tels, on veut juste les prendre en compte.

### Passons au modèle mixte pour prendre en compte les interactions :

Dans un modèle mixte, on fait l’hypothèse que les interactions reflètent des **variations aléatoires d’un même processus**, et non des processus distincts.
...
On opte pour un modèle à **pentes aléatoires sur les effets fixes** = une pente intra-massif, moyenne sur tous les massifs ($\alpha$, $\beta$, $\gamma$, $\delta$) + une variation inter-massif ($\theta$, $\rho$, $\gamma$, $\omega$). Cela correspond aux interactions dans un LM ou un GLM.

\[ log(\lambda_i) = \alpha_j + \beta_j \times Hdom_i + \gamma_j ST_i + \sum_{k = 2}^{k}  \delta_{k,j} \times essence_i + \theta_i  \]
- Effet aléatoire “massif forestier” :
\[ \theta_i \sim N(\mu_j, \sigma) \]
